package com.br.formulario.util.managerbean.diversos;
...
public class FacesUtil {
	...
	public static String obterParametroDaTela(String idComponente) {
	...
	public static String obterAtributo(ActionEvent event, String name) {
		return (String) event.getComponent().getAttributes().get(name);
	}
	...
	/** seta o valor a ser guardado e ser resgatado posteriormente com o getFlash. A duração desses
	 * objetos encaminhados pelo setFlash será até a página de destino ser renderizada, após isso
	 * eles serão ignorados. Dessa forma é recomendado utilizar durante a iniciação do MB ou com o @postconstruct
	 * @param parametro: chave que identificar a informação armazenada
	 * @param valor: valor a ser guardado */
	public static void setFlash(String parametro, Object valor) {
		FacesContext.getCurrentInstance().getExternalContext().getFlash().put(parametro, valor);
	}


	/** Resgata o um valor guardado com o setFlash
	 * @param parametro: chave que identificar a informação armazenada
	 * @return */
	public static Object getFlash(String parametro) {
		Object resultado = FacesContext.getCurrentInstance().getExternalContext().getFlash()
				.get(parametro);
