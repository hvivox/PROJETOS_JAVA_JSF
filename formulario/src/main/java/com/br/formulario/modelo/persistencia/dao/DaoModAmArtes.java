...
public class DaoModAmArtes extends DataAccessObject<ModAmArtes> {
	...
	@Inject
	public DaoModAmArtes(EntityManager manager) {
		super(ModAmArtes.class, manager);
	}
	...
	public List<Object[]> consultarDadosInscricao(FiltroPesquisaPadrao filtro)
		...
		Query query = getManager().createQuery(
				"select mod, count(amArtes) as contador "
						+ "FROM AmazonasArtes amArtes RIGHT JOIN amArtes.modAmArtes mod WITH "
						+ "(amArtes.dtaInscricao >= :dtaInscricaoParametro) " + "WHERE  "
