...
@Named("mbCadastroUnidade")
@ViewScoped
public class MBCadastroUnidade implements Serializable {
	...
	private static final String caminhoFisicoImagemUnidade = "C:/Program Files/PainelMobileImagem/unidade";
	// redireciona para o caminho abasoluto da imagem
	private static final String linkSemNomeDaImagem = "http://www.sesc-am.com.br:8090/painelmobile/webservice/imagem/unidade";


	public MBCadastroUnidade() {
		limparFormulario();
	...
	public void inicializar() {
		...
		if (FacesUtil.isNotPostBack()) {
			...
			if ((unidade instanceof Object) && unidade != null) {
				
				System.out.println("ENVIOU!! ###############################");

				// CONFIGURA O GER IMAGEM PARA RECEBER EXIBIR UMA IMAGEM DO DISCO
				mbGerimagem.parametrizarGerenciador(unidade.getId(),
						caminhoFisicoImagemUnidade, unidade.getNomeImagem(), null);
			}
		}
	}
	...
	public void salvar() {
		try {
			...
			//VERIFICAR SE HÁ UMA INSTANCIA VALIDA PARA VALIDAR O unidade.getId()
			if ((unidade instanceof Object)) {
				// NAO HOUVE O UPLOAD
				if (!(mbGerimagem.getUploadFile() instanceof Object)) {
					
					// MODO EDITRAR - GRAVA AS INFORMAÇÕES
					if (unidade.getId() != null && unidade.getId() > 0) {
						unidade = serviceUnidade.salvarOuAtualizar(unidade);
						limparFormulario();
						limparImagem();
	...
	public void gravarImagemNoDisco() {		
		...
		try {
			...
			caminhoFisicoImagemUnidade, uploadFile);
	 ...
	 * @param caminhoAbsoImagemUnidade: recebe String neste formato ex: :C:\diretorio\
	 * @return Path nesse formato ex:C:\diretorio\imagem0239223.jpg
	 * @throws IOException */
	public Path gerarCaminhoFisicoComNomeDoArquivo(String caminhoAbsoImagemUnidade,
			UploadedFile uploadFile) throws IOException {
		Path caminhoAbsoluto = Paths.get(caminhoFisicoImagemUnidade);
		...
		unidade.setNomeImgOriginal(uploadFile.getFileName());
		unidade.setNomeImagem(novoNomeUpload);
		unidade.setUriImagem(linkSemNomeDaImagem + "/" + novoNomeUpload);
	...
	private void limparImagem() {
		...
		mbGerimagem.parametrizarGerenciador(0, caminhoFisicoImagemUnidade, "", null);
	}
	...
	public void setUnidade(Unidade unidade) {
		System.out.println("chama o SETunidade..................");
		this.unidade = unidade;
		/* if(unidade != null){ this.listaEstatus = EstatusEntidadeEnum.values(); } */
	}
