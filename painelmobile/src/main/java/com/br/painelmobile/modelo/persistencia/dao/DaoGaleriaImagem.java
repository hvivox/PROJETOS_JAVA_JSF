...
public class DaoGaleriaImagem extends	DataAccessObject<GaleriaImagem> {
	
	private static final long serialVersionUID = 1L;
	
	@Inject
	public DaoGaleriaImagem(EntityManager manager) {
		super(GaleriaImagem.class, manager);
	}
	...
	public List<GaleriaImagem> consultarPorFiltrados(FiltroPesquisaPadrao filtro)
			throws ObjetoNaoEncontradoException {
		Session session = manager.unwrap(Session.class);
		Criteria criteria = session.createCriteria(GaleriaImagem.class);
		...
		List<GaleriaImagem> resultado = criteria.addOrder(Order.desc("id")).list();
